...
export default async function CareersPage() {
  ...
  const DEFAULT_BENEFITS: BenefitItem[] = [
    ...
    { text: 'Join a team that values integrity, innovation, and collaboration', iconKey: 'users' },
    { text: 'Opportunities for career growth', iconKey: 'growth' },
    { text: 'Inclusive, diverse, and empowering environment', iconKey: 'sparkles' },
  ];
  ...
  return (
    <main className="min-h-screen bg-[#f5f7fa] text-gray-900 font-sans">
      ...
      <UnifiedHero
        ...
        subtitle={page?.intro ?? 'We’re a community of innovators, builders, and changemakers.'}
        ...
        <div className="mx-auto max-w-7xl">
          {(page?.jobsHeading || page?.jobsSubtext) && (
            <div className="mb-4 text-center">
              ...
              {page?.jobsSubtext && (
                <p className="mt-1 text-sm text-[#586270]">{page.jobsSubtext}</p>
              )}
            </div>
          )}
      ...
      {/* Internships */}
      <InternshipPrograms title={page?.internshipTitle} body={page?.internshipBody} />

      {/* How to apply */}
      <HowToApply title={page?.howToApplyTitle} steps={page?.howToApplySteps ?? page?.howToApply} />
